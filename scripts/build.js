...
import parseArticle from './tasks/parse_article.js';
...
for (let i = 0, { length } = articleIdList; i < length; i += 1) {
  ...
  const html = await ejs.renderFile(articleTemplate, { ...data, config });
...
const indexHtml = await ejs.renderFile(indexTemplate, {
